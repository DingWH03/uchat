...
pub async fn handle_members_group(
    ...
    let session_id = if let Some(session_id_cookie) = cookies.get("session_id") {
        ...
        warn!("未找到 session_id Cookie，查询群聊成员操作");
        return (
            axum::http::StatusCode::UNAUTHORIZED,
            "未找到 session_id Cookie，查询群聊成员操作",
        )
            .into_response();
    };
    ...
    let result = request_lock.get_group_members(payload.id).await;
    let response = match result {
        Ok(members) => ServerResponse::GroupMembers { group_id: payload.id, member_ids: members } ,
        Err(e) => {
            warn!("用户{}查询群聊成员出现错误: {}", user_id, e);
            ServerResponse::GenericResponse { status: "Err".to_string(), message: "服务器错误".to_string() }
        }
    };
    debug!("查询群聊成员请求响应: {:?}", response);
