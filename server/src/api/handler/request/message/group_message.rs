...
pub async fn handle_get_group_message(
    ...
    let session_id = if let Some(session_id_cookie) = cookies.get("session_id") {
        ...
        warn!("未找到 session_id Cookie，拒绝获取群聊聊天记录");
        return (
            axum::http::StatusCode::UNAUTHORIZED,
            "未找到 session_id Cookie，拒绝获取群聊聊天记录",
        )
            .into_response();
    };
