...
use crate::{
    protocol::{manager::{DeleteMessageRequest, GetMessageRequest, GetRecentMessageRequest, GetUserRecentMessageRequest}, Empty, FullPrivateMessage, ManagerResponse, PreviewPrivateMessage},
    server::AppState,
};
...
pub async fn handle_get_recent_messages(
    ...
    match manager_lock.check_session_role(&session_id).await {
        Some(role) if role.is_admin() => {
            manager_lock.get_recent_messages(payload.count, payload.offset).await.into_response()
        }
...
pub async fn handle_get_user_recent_messages(
    ...
    match manager_lock.check_session_role(&session_id).await {
        Some(role) if role.is_admin() => {
            manager_lock.get_user_recent_messages(payload.count, payload.offset, payload.user_id).await.into_response()
        }
        Some(_) => ManagerResponse::<()>::forbidden().into_response(),
        None => ManagerResponse::<()>::unauthorized().into_response(),
    }
...
pub async fn handle_delete_message(
    ...
    match manager_lock.check_session_role(&session_id).await {
        Some(role) if role.is_admin() => {
            manager_lock.delete_message(payload.message_id).await.into_response()
        }
        Some(_) => ManagerResponse::<()>::forbidden().into_response(),
        None => ManagerResponse::<()>::unauthorized().into_response(),
    }
...
pub async fn handle_get_message(
    ...
    match manager_lock.check_session_role(&session_id).await {
        Some(role) if role.is_admin() => {
            manager_lock.get_message(payload.message_id).await.into_response()
        }
        Some(_) => ManagerResponse::<()>::forbidden().into_response(),
        None => ManagerResponse::<()>::unauthorized().into_response(),
    }
