...
pub trait UserDB: Send + Sync {
    ...
    /// 更新用户密码
    async fn update_password(&self, id: u32, new_password_hash: &str) -> Result<(), sqlx::Error>;
    /// 创建新用户
...
pub trait MessageDB: Send + Sync {
    ...
    /// 添加群聊信息聊天记录
    async fn add_group_message(&self, group_id: u32, sender: u32, message: &str) -> Result<u64>;
    /// 获取私聊聊天记录
    ...
    /// 返回值为元组，元组的第一个元素是发送者的id，第二个元素是timestap，第三个元素是消息内容
    /// offset是消息分组，一组消息30条，0代表最近的30条，1代表30-60条，以此类推
    async fn get_group_messages(&self, group_id: u32, offset: u32) -> Result<Vec<SessionMessage>>;
    /// 获取某群聊最新一条消息时间戳
    async fn get_latest_timestamp_of_group(&self, group_id: u32) -> Result<Option<NaiveDateTime>>;
    /// 用户加入群聊的所有的群消息最后的时间戳
